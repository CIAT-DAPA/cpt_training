...
tsm_list_table <- lapply(tsm_raster,function(x1)lapply(x1,function(x) t(rasterToPoints(x))[-1:-2,]))
...
lat <-lapply(tsm_raster,function(x1)lapply(x1,function(x) t(rasterToPoints(x))[2,]))
ponde <-lapply(lat,function(x1)lapply(x1,function(x) sqrt(cos((pi/180)*x))))
